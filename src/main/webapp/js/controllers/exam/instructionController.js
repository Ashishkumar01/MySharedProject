...
IndexModule.controller("instructionController", function($rootScope,$scope,$location,$http,$routeParams,examsService) {
	  ...
	  if($.cookie("validAdminClick")=="ok" || $scope.beginExamClicked == true)
	  {
	    ...
	    if($routeParams.id){
	        ...
	        $http({method: 'POST', url: 'rest/set/examset/'+$routeParams.id, data:$scope.userId})
	    ...
	    $scope.viewReports=function(){
	        $scope.beginExamClicked = true
	        $location.path('/viewReports/'+$rootScope.currentExam.examSetId+'/'+$rootScope.currentExam.currentAttempt+'/'+$scope.userId);
	    }
